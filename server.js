...
io.on('connection', (socket) => {
    ...
    socket.on('run_preloaded', async (data) => {
        ...
        console.log(`[${socket.id}] RUN_PRELOADED: ${scriptId} | ${url}`);
        ...
        try {
            if (scriptId === 'example_scrape_title') {
                ...
                console.log(`[${socket.id}] [1/5] Launching browser...`);
                socket.emit('status', { step: 1, message: 'Launching browser...' });

                const browser = await launchBrowserWithStealth();
                const { context, page } = await createStealthPage(browser);

                try {
                    console.log(`[${socket.id}] [2/5] Navigating...`);
                    socket.emit('status', { step: 2, message: 'Navigating to page...' });
                    ...
                    console.log(`[${socket.id}] [3/5] Waiting for page load...`);
                    socket.emit('status', { step: 3, message: 'Waiting for page to load...' });

                    await new Promise(resolve => setTimeout(resolve, 5000));

                    console.log(`[${socket.id}] [4/5] Searching for shortlink...`);
                    socket.emit('status', { step: 4, message: 'Searching for shortlink...' });

                    const result = await waitForValue(page, 45000);
                    const targetUrl = result.value;

                    console.log(`[${socket.id}] [5/5] Executing action...`);
                    socket.emit('status', { step: 5, message: 'Executing action...' });
                    ...
                    socket.emit('preloaded_success', {
                        message: 'AnimeDekho verification completed successfully!'
                    });
            ...
            console.error(`[${socket.id}] ERROR in run_preloaded:`, error.message);
